...
{
    if([key isEqualToString:@"description_detail"])
    {
        return @"description";
    }
    ...
    if ([key isEqualToString:@"hotid"]) {
        return @"id";
    }
    //isnewDynamic
    if ([key isEqualToString:@"isnewDynamic"]) {
        return @"newDynamic";
    }

    if ([key isEqualToString:@"INITResult"]) {
        return @"initResult";
    }
    
    if ([key isEqualToString:@"INITResultMsg"]) {
    ...
    if(self && [dict isKindOfClass:[NSDictionary class]]) {
        ...
        for(NSDictionary *keyAndtype in propertyArray)
        {
            [keyAndtype enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
                ...
                {
                    //得到下级数据的字典
                    id obj = [dict objectForKey:[self checkTheKeyDataSource:key]];
                ...
                else if([Subclass isSubclassOfClass:[NSArray class]])
                {
                    ...
                    if([obj isKindOfClass:[NSArray class]])
                    {
                        ...
                        {
                            if([SetSubClass isSubclassOfClass:[BaseModel class]])
                            {
                ...
                else if ([Subclass isSubclassOfClass:[NSNumber class]]){
                    
                    NSString *dictKey = [self checkTheKeyDataSource:key];
                    id obj = [dict objectForKey:dictKey];
                    if([obj isKindOfClass:[NSArray class]])
                    {
